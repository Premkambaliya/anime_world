...
const DemonSlayer = () => {
  ...
  const [likes, setLikes] = useState(Number(localStorage.getItem("likes_demonslayer")) || 0);
  const [shares, setShares] = useState(Number(localStorage.getItem("shares_demonslayer")) || 0);
  ...
  useEffect(() => {
    axios
      .get("https://anime-world-1.onrender.com/anime/6799dff5d4902dee19968a7a")
      .then((response) => {
  ...
  const handleLike = () => {
    ...
    setLikes(newLikes);
    localStorage.setItem("likes_demonslayer", newLikes);
  };
  ...
  const handleShare = () => {
    ...
    setShares(newShares);
    localStorage.setItem("shares_demonslayer", newShares);
    ...
    <div className="flex justify-center items-center min-h-screen bg-[#121212] px-5 relative">
      <div className="max-w-3xl bg-[#1e1e1e] rounded-xl p-6 shadow-lg">
        ...
        <div className="text-center">
          <img
            src="https://res.cloudinary.com/doqzxuxb1/image/upload/v1738671712/wm3rotcikovo3hnlebye.png"
            alt={anime.title}
